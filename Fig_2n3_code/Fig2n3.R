...
tax_df <- rbind(read.delim(file = "inputs/taxonomy/integrated_tax_df_Bacteria_genomes.txt"),
...
models_df_Bacteria <- rbind(read.delim(file = "inputs/model_fitness/OUT_Model_fitness_5end_ALL_1_to_3000.txt"),
...
ORFeome_features <- rbind(read.delim(file = "inputs/ORFeome_features/ORFeome_features_Archaea.txt"),
...
Fig_S1A <- integrated_df %>% 
  ggplot(mapping = aes(x      = superkingdom,
                       y      = Length_total,
                       fill = superkingdom)) +
...
Fig_S1D <- integrated_df %>%
  filter(phylum != "NA") %>% 
  ggplot(mapping = aes(x      = reorder(phylum, GC_mean, FUN=mean),
                       y      = GC_mean*100, # Value
                       colour = superkingdom)) +
  geom_hline(yintercept = 50, lty=2) +
  stat_summary(fun.y = mean, fun.ymax = function(x){mean(x)+(1*sd(x))}, fun.ymin = function(x){mean(x)-(1*sd(x))}, geom = "errorbar") +
...
Fig_S1 <- plot_grid(Fig_S1ABC, Fig_S1D, Fig_S1E, labels = c("", "D", "E"), label_size = 22, ncol = 1, nrow = 3, rel_heights = c(1, 1.75, 1.75))
...
Fig_S2D <- integrated_df %>%
  ...
  ggplot(mapping = aes(x      = reorder(phylum, GC_mean, FUN=mean),
                       ...
                       lty = Modeling)) +
  geom_hline(yintercept = 50, lty=2) +
  stat_summary(fun.y = mean, fun.ymax = function(x){mean(x)+(1*sd(x))}, fun.ymin = function(x){mean(x)-(1*sd(x))}, geom = "errorbar", position=position_dodge(width=0.75)) +
...
Fig_2F <-integrated_df %>% 
  ggplot(aes(x = No_of_CDSs, # No_of_CDSs , Length_total
             y = -log10(P_Rate),
             #y = Rate,
             colour= superkingdom)) +
...
ggsave(plot = plot_grid(Fig_2A, Fig_2B, Fig_2C, Fig_2D, Fig_2E, Fig_2F, labels = c("A", "B", "C", "D", "E", "F"), label_size = 8, ncol = 3, nrow = 2),
...
Fig_3A <- ggplot(data = integrated_df,
                 mapping = aes(x = superkingdom, y = Carrying_capacity)) + #  "Carrying_capacity" "Rate" "Initial_cost"
  ...
  stat_summary(fun.y = median, fun.ymin = function(x){median(x)-sd(x)}, fun.ymax = function(x){median(x)+sd(x)}, colour = "black", geom = "pointrange", size=0.25) +
...
Fig_3B <- ggplot(data = integrated_df,
                 mapping = aes(x = superkingdom, y = Initial_cost)) + #  "Carrying_capacity" "Rate" "Initial_cost"
  #geom_hline(yintercept = 0, lty=2, colour="black") +
  geom_violin(aes(colour=superkingdom, fill=superkingdom), alpha=0.5, size=0.25) +
  stat_n_text(y.expand.factor = 0.2, geom = "text", fontface = "italic", vjust = 0, size = 2, color="grey") +
  stat_summary(fun.y = median, fun.ymin = function(x){median(x)-sd(x)}, fun.ymax = function(x){median(x)+sd(x)}, colour = "black", geom = "pointrange", size=0.25) +
...
Fig_3C <- ggplot(data = integrated_df %>% 
                   filter(Modeling == "Successful"),
                 mapping = aes(x = superkingdom, y = Rate)) + # superkingdom         phylum          class           order            family           genus
...
Fig_3D <- ggplot(data = integrated_df, #%>% 
                 #filter(Modeling == "Successful"),
...
ggsave(plot = plot_grid(Fig_3A, Fig_3B, Fig_3C, Fig_3D, labels = c("A", "B", "C", "D"), label_size = 8, ncol = 2, nrow = 2),
       filename = "Fig_3.pdf",
       device = "pdf",
       width = 89,
       height = 89,
